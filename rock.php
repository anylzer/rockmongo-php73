...
function rock_name_to_java($name) {
	$name = preg_replace_callback('/_([a-zA-Z])/', function($matches){return strtoupper($matches[1]);}, $name);
...
function rock_array_get(array $array, $keys) {
	...
	if (!is_array($keys)) {
		if (strstr($keys, "`")) {
			...
			$keys = preg_replace_callback("/`(.+)`/U", function($matche){return str_replace(".", "\.", $matche[1]);}, $keys);
		}
